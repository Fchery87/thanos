...
There is no single `resolveConfig` entry point. Each surface is loaded by its
own module, with its own precedence and trust rules:

| Surface | Loader | Documented in |
|---|---|---|
| Delivery mode, autonomy, yolo lock | `resolveDeliveryState` (`src/governance/delivery.ts`) | [governance.md](governance.md#delivery-modes) |
| Policy rules and presets | `loadPolicyState` (`src/policy/state.ts`) | [governance.md](governance.md) |
| `/goal` defaults | `loadGoalSettings` (`src/goal/load-settings.ts`) | this file, above |

A `src/config/resolve.ts` composing all three into one `ResolvedConfig` was
written on 2026-07-23 (`d82d5a5`) and deleted on 2026-07-27 without ever having
been called. Its plan had six steps — write test, verify fail, implement, verify
pass, document, commit — and no step that wired it to anything, so it shipped
unreachable and stayed that way. It is recorded here because this section
documented it as though it were live, which is the more expensive half of the
mistake.
