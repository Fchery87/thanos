...
**Thanos**
The Pi config/harness layer living at `~/.pi`. Thanos is now intended to grow from a governance extension into a broader Oh-my-pi-style **Agent Distribution** while preserving governance as a first-class differentiator. Distributed at `github.com/fchery87/thanos` with a low-friction bootstrap installer whose payload is always pinned to a release tag (git-based install, ADR 0008); only the small bootstrap script itself is fetched from the branch tip, an accepted tradeoff recorded in that ADR.
...
- **Todo persistence**: Keep todos session-local by default; only write project files through explicit export/import. Reconstructing todo state from the session branch/transcript (so it survives reload) is session-local durability, not a project-file write, and is consistent with this decision.
...
- **Distribution**: Keep the bootstrap install UX. Both installers use the git-based model of ADR 0008 (which supersedes ADR 0002's SHA256-tarball design): fresh installs `git clone` and check out the latest release tag; `thanos update` fetches tags and advances to the latest release tag without touching user-owned gitignored files. Teams pin with `--ref`/`THANOS_REF=vX.Y.Z`. Release integrity comes from tag pinning over HTTPS after the full CI gate; the bootstrap script itself is fetched from the branch tip as an accepted, ADR-recorded tradeoff (a fully pinned chain is documented in `docs/install.md`). Signatures can be added later when release key management is mature.
- **Installer verification**: Treat `scripts/install.sh` as a security boundary. Add static validation when available and fixture-driven tests for release-tag resolution, pinned refs, update behavior that preserves user files, and refusal of non-Thanos directories.
...
- Installer distribution has been re-resolved by ADR 0008 (superseding ADR 0002's SHA256-tarball resolution) as a git-based latest-release-tag install on all platforms with an explicit `--ref`/`THANOS_REF` override — not branch-tip payloads, silent latest installs, Pi version pinning, or signature verification from day one.
- `thanos update` updates to the latest release tag by default and respects `--ref`/`THANOS_REF` for pinned updates, never touching user-owned gitignored files.
- README/install docs may advertise the `master`-tip bootstrap one-liner because the installed payload is tag-pinned regardless; the fully pinned alternative (installer fetched from a release tag) must stay documented alongside it.
