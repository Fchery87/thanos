// The pi-subagents version-pin orchestration logic, split out from
// scripts/thanos-update.mjs so it can be ESM-imported directly by
// tests/scripts/update.test.ts without importing a shebang-prefixed file.
// scripts/thanos-update.mjs's shebang IS load-bearing (it's the
// package.json "bin" entry for `thanos-update`), unlike
// scripts/patch-pi-subagents.mjs's (never executable, always invoked via an
// explicit `node`), so deleting the shebang wasn't an option here — a
// shebang-prefixed .mjs file fails to parse on Windows CI when ESM-imported
// directly (esbuild/vite-node's shebang handling chokes there; every other
// existing test that touches a shebang script spawns it as a subprocess or
// writes a fake file at its path, never imports it directly — this was the
// first direct import of one, and the first to hit it).
//
