...
/**
 * session_start (persisted-goal restore, welcome header, MCP init,
 * first-launch delivery selector, status-bar segments), session_tree (todo
 * state re-sync on branch switch), and session_shutdown (MCP disconnect — the
 * natural counterpart to the MCP init this module already owns). All three
 * share mcpManager/todoRuntime state, so extracted together.
 */
