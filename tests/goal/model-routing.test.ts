...
import { parseModelRef, pickRoleModel, resolveRoleModelAuth } from "../../src/goal/model-routing";
...
describe("pickRoleModel", () => {
  ...
  it("returns undefined when nothing in the chain is usable", () => {
    expect(pickRoleModel({ model: "nope/missing" }, registry, allAuthed)).toBeUndefined();
    expect(pickRoleModel({ model: "not-a-ref" }, registry, allAuthed)).toBeUndefined();
  });
});

describe("resolveRoleModelAuth", () => {
  ...
  it("returns apiKey and headers from the registry resolution", async () => {
    ...
    await expect(resolveRoleModelAuth(registry, model)).resolves.toEqual({
      apiKey: "sk-123",
      headers: { "x-a": "b" },
    });
  });
  ...
  it("throws with provider/model context when resolution fails", async () => {
    ...
    await expect(resolveRoleModelAuth(registry, model)).rejects.toThrow(
