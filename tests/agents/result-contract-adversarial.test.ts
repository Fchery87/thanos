...
describe("parseSubagentResult — adversarial inputs", () => {
  it("handles oversized JSON input gracefully", () => {
    const large = JSON.stringify({ version: 1, summary: "x".repeat(600_000) });
  ...
  it("handles deeply nested metadata safely", () => {
    ...
    const input = JSON.stringify({ version: 1, summary: "test", metadata: deep });
  ...
  it("handles oversized metadata bytes safely", () => {
    ...
    const input = JSON.stringify({ version: 1, summary: "test", metadata: { data: large } });
    const result = parseSubagentResult(input);
    expect(result.metadata).toBeUndefined();
  });

  it("rejects unknown status values", () => {
    const input = JSON.stringify({ version: 1, summary: "ok", status: "exploited" });
    const result = parseSubagentResult(input);
    expect(result.status).toBe("error");
  });
  ...
  it("bounds findings array size", () => {
    ...
    const input = JSON.stringify({ version: 1, summary: "test", findings });
    const result = parseSubagentResult(input);
  ...
  it("bounds escalations array size", () => {
    ...
    const input = JSON.stringify({ version: 1, summary: "test", escalations });
    const result = parseSubagentResult(input);
    expect(result.escalations.length).toBeLessThanOrEqual(10);
  });

  it("handles non-object JSON values", () => {
    expect(parseSubagentResult("42").status).toBe("error");
    expect(parseSubagentResult('"just string"').status).toBe("error");
  });

  it("handles empty and whitespace strings", () => {
    const result = parseSubagentResult("");
    expect(result.status).toBe("error");
    expect(result.summary).toBe("invalid result contract format");
    expect(result.metadata).toEqual({ legacy: true });
  });

  it("truncates summary exceeding max length", () => {
    const longSummary = "x".repeat(5000);
    const input = JSON.stringify({ version: 1, summary: longSummary });
    const result = parseSubagentResult(input);
  ...
  it("allows explicit legacy adapters to opt into plain text", () => {
    expect(parseSubagentResult("legacy prose", { legacyAdapter: true }).status).toBe("success");
  });
});
